...
if (-not (Test-Path (Join-Path $repoRoot "Primitives.sln"))) {
    $parentRoot = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path

    if (Test-Path (Join-Path $parentRoot "Primitives.sln")) {
        $repoRoot = $parentRoot
    }
    else {
        throw "Unable to locate Primitives.sln from script path '$PSScriptRoot'."
    }
}
